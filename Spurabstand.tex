...
Aufgrund der Funktionsweise des Leseverfahrens, ist es naheliegend, ein ähnliches Verfahren zur Ermittlung des Spurabstandes von optischen Datenträgern zu nutzen. Dabei können die Vertiefungen in der Kunststoffschicht als Doppelspalt genutzt werden. Um davon auf den Spurabstand schließen zu können, ist es erforderlich dass von Löchern auf verschiedenen Spuren simultan bestrahlt werden. Wegen dem daraus resultierenden Größenverhältnis zwischen Spurabstand und Strahlgröße ist es notwendig, einen schwach gebündelten Laserstrahl zu verwenden. Zur Erfassung der abgelenkt und reflektierten Strahlen wird eine Mattscheibe genutzt, auf welcher die Abstände der Strahlen zueinander gemessen werden können, woraus sich folgender vereinfachter Versuchsaufbau ergibt \ref{fig:Versuchsaufbau}.
...
Für den Spurabstand einer CD ergibt sich dann mit \eqref{eq:Spurabstand_Sinus}:
\begin{align*}
 g_{cd} &= \frac{632\cdot10^{-9}\mbox{ m}^2}{\sin{20,8}}\\
 &\approx 1,78 \cdot 10^{-9}\mbox{m}
\end{align*}
Analog dazu der Spurabstand der DVD mit \eqref{eq:Spurabstand_Sinus}:
\begin{align*}
 g_{dvd} &= \frac{632\cdot10^{-9}\mbox{ m}^2}{\sin{60,0}}\\
